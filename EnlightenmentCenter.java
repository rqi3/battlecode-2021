package my_player_nathan_bidder_theoretical;
...
public class EnlightenmentCenter {
    ...
    */
    ...
    static boolean necessaryComeback = false;
    static int getBidValue(){ //returns the value this Enlightenment Center will bid
    	int us = rc.getTeamVotes();
    	int them = rc.getRoundNum() - rc.getTeamVotes(); //might be slightly overestimated in the case of ties
    	
    	if(necessaryComeback) return 2*rc.getInfluence()/3;
    	
    	if(us > 1500) return 0; //we have majority vote, just invest in full defense
    	if(them > 1525) return 0; //they have majority vote, invest in full offense
    	
    	if(rc.getRoundNum() > 2750) return 2*rc.getInfluence()/3; //end game, get those last votes - not much point in building economy rn
    	if(them - us > 300) {
    		necessaryComeback = true;
    		return 2*rc.getInfluence()/3; //hopefully our economy is caught up enough at this point otherwise it's just lost
    	}
        
    	if(us - them > 100) { // economic play
    		necessaryComeback = false;
    		return (int) (Math.random()*50);
    	}
    	if(them - us > 100) { // offensive play
    		return (int) (Math.random() * 50);
    	}
    	
    	//lots of money in bids... try to come ahead in these close scores
    	return 2*rc.getInfluence()/3;
    }
