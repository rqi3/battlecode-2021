...
////////////////
	...
	private static void doECAttackerAction() throws GameActionException {
		...
		boolean canMoveAnyDirection=false;
		...
		if(!moveAction){
			stuck_time++;
			int radius = Math.min(distance_to_target, RobotType.POLITICIAN.actionRadiusSquared);


			if(stuck_time>3&&distance_to_target>RobotType.POLITICIAN.actionRadiusSquared)
			{
				for(Direction dir: directions){
					if(rc.canMove(dir))
					{
			...
			if(stuck_time>10||(!canMoveAnyDirection))
			{
			...
			for(RobotInfo r : rc.senseNearbyRobots(distance_to_target)) {
			...
			if(distance_to_target<=RobotType.POLITICIAN.actionRadiusSquared&&num_nearby_enemies<=2*EmpowerFactor)
			{
	...
	public static void run() throws GameActionException{
		...
		switch(politician_type)
		{
			case LOST_POLITICIAN:
				System.out.println("im a LOST_POLITICIAN");
				doLostPoliticianAction();
				break;
			case EC_ATTACK:
				System.out.println("im a EC_ATTACK");
				doECAttackerAction();
				break;
			case POLICE:
				System.out.println("im a POLICE");
				doPoliceAction();
				break;
			default:
