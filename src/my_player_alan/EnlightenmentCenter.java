...
public class EnlightenmentCenter {
	...
	static int attackMuckrackerTargetInfluence=100;
	public static void spawnRobot() throws GameActionException {
		save_money = true; //this will be set true within the spawnRobot method if our strategy involves saving money for some reason
		...
		{
			if(ClosestEnemyAttacker.enemy_position.distanceSquaredTo(rc.getLocation())<100)
			{
				if(ClosestEnemyAttacker.enemy_type==1)
				{
					very_close_muckraker = true;
					very_close_muckraker_max_conviction = Math.max(very_close_muckraker_max_conviction, 10);
				}
			}
		}
	...
	if round <= 100
		...
		if(rc.getRoundNum() <= 100) {
			...
			if(rc.getInfluence()>=attackMuckrackerTargetInfluence*2)
				if(trySpawnAttackerMuckraker(attackMuckrackerTargetInfluence,null))
					attackMuckrackerTargetInfluence=attackMuckrackerTargetInfluence*3/2;
