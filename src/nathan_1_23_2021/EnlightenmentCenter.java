...
public class EnlightenmentCenter {
	...
	public static void spawnRobot() throws GameActionException {
		...
		boolean very_close_politician = false;
		int close_politician_conviction = -1;
		for(RobotInfo close_enemy_unit: rc.senseNearbyRobots(40, rc.getTeam().opponent())){
			...
			if(close_enemy_unit.getType() == RobotType.POLITICIAN) {
				very_close_politician = true;
				close_politician_conviction = Math.max(close_politician_conviction, close_enemy_unit.getConviction());
			}
			close_enemy = true;
		}
		int bound = very_close_politician ? rc.getInfluence() - (close_politician_conviction+1)/2 : rc.getInfluence();

		if(num_surrounders >= 2){
		...
		if(possible_threat) police_to_slanderer = 2.0;
		
		if(rc.getRoundNum() <= 100) {
			if(alive_slanderer_ids.size() < 2) {
				if(rc.getInfluence() >= 85) {
					trySpawnSlanderer(getOptimalSlandererInfluence(rc.getInfluence()));
				} else {
					trySpawnCheap();
				}
			} else {
				if(rc.getInfluence() >= 107 && alive_slanderer_ids.size() < alive_police_politician_ids.size()) { 
			...
			if(cycle % 3 == 2) {
				if(RobotPlayer.neutral_ecs.size() > 0) {
					...
					int attacker_politician_influence = ec_target_influence+600;
					...
					if(RobotPlayer.enemy_ecs.size() > 0 && rc.getInfluence() > 500) {
						if(Math.random() < 0.8) {
							...
							attacker_politician_influence = Math.min(200, (int)(rc.getInfluence()/3));
