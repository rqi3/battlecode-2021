	...
	public static void spawnRobotCloseEnemyCase() throws GameActionException{
		...
		for(RobotInfo close_enemy_unit: rc.senseNearbyRobots(4, rc.getTeam().opponent())){
			if(close_enemy_unit.getType() == RobotType.POLITICIAN){
				allowance = Math.min(allowance, rc.getInfluence()-(int)(close_enemy_unit.getConviction()*rc.getEmpowerFactor(rc.getTeam().opponent(),0)));
			}
		}
		...
		if(surround_enemies >= 4||(surround_enemies>=1&&total_space<8)){
		...
		for(int i = 1; i < RobotPlayer.enemy_ecs.size(); i++){
			...
			if(this_dist < cur_dist){
				...
				closest_enemy_ec_id=i;
			}
		}
		int enemyECInfluence=RobotPlayer.enemy_ecs.get(closest_enemy_ec_id).influence;

		int influenceNeeded=(int)((enemyECInfluence+50)/rc.getEmpowerFactor(rc.getTeam(),20)+1);

		if(influenceNeeded<allowance)
		{
			trySpawnAttackerPolitician((influenceNeeded*2+allowance)/3,closest_enemy_ec);
		}
		...
		if(alive_scout_ids.size() < 8&&alive_attack_muckraker_ids.size() <= 15)
		{
			if(Math.random()<0.5) trySpawnAttackerMuckraker(1, closest_enemy_ec);
			else trySpawnScout();
		}
		if(alive_attack_muckraker_ids.size() <= 15){
			trySpawnAttackerMuckraker(1, closest_enemy_ec);
		}
